...
<html lang="en">
<head>
    <title>Code coverage report for Relayer.js</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
  <div class='pad1'>
    <h1>
      <a href="index.html">All files</a> Relayer.js
    </h1>
    <div class='clearfix'>
      <div class='fl pad1y space-right2'>
        <span class="strong">75.41% </span>
        <span class="quiet">Statements</span>
        <span class='fraction'>92/122</span>
      </div>
      <div class='fl pad1y space-right2'>
...
<span class="cline-any cline-neutral">&nbsp;</span>
<span class="cline-any cline-neutral">&nbsp;</span>
<span class="cline-any cline-neutral">&nbsp;</span></td><td class="text"><pre class="prettyprint lang-js">import Web3 from 'web3';
...
&nbsp;
export class Tx {
    constructor(txHash, toHomeBridge, data = <span class="branch-0 cbranch-no" title="branch not covered" >{})</span> {
        this.txHash = txHash;
...
&nbsp;
        <span class="missing-if-branch" title="if path not taken" >I</span>if (!tx.sideToken) {
<span class="cstat-no" title="statement not covered" >            this.updateTxData(</span>
                Object.assign({}, tx, {
                    status: 'failed-send',
...
&nbsp;
                ...
                <span class="missing-if-branch" title="else path not taken" >E</span>if (!txHash) {
...
&nbsp;
    sendHomeTx(tx, gasPrice) {
        ...
        return this.nonceTracker
            ...
            .catch(<span class="fstat-no" title="function not covered" >(e</span>rror, receipt) =&gt; {
<span class="cstat-no" title="statement not covered" >                logger.debug('HomeBridge tx error -&gt;', error, receipt, txHash);</span>
&nbsp;
                // if we have a homeTxHash, then we will pick up the failure in the Verifyer
<span class="cstat-no" title="statement not covered" >                if (!txHash) {</span>
<span class="cstat-no" title="statement not covered" >                    this.nonceTracker.releaseNonce(nonce, true, false);</span>
<span class="cstat-no" title="statement not covered" >                    this.updateTxData(</span>
                        Object.assign({}, tx, {
                            status: 'failed-send',
                            error,
...
&nbsp;
                return Promise.all([
                    ...
                    .then(async ([toForeignTxs = <span class="branch-0 cbranch-no" title="branch not covered" >[],</span> toHomeTxs = <span class="branch-0 cbranch-no" title="branch not covered" >[]]</span>) =&gt; {
...
&nbsp;
                        <span class="missing-if-branch" title="else path not taken" >E</span>if (this.config.isTest) {
...
<span class="cstat-no" title="statement not covered" >                        logger.error('Error occured -&gt;', err);</span>
<span class="cstat-no" title="statement not covered" >                        this.bridgeData.homeBlockLastRelayed = homeFromBlock;</span>
<span class="cstat-no" title="statement not covered" >                        this.bridgeData.foreignBlockLastRelayed = foreignFromBlock;</span>
<span class="cstat-no" title="statement not covered" >                        this.updateBridgeData(this.bridgeData);</span>
                    });
            })
            .catch(err =&gt; {
...
<span class="cstat-no" title="statement not covered" >                    logger.error('Error loading bridge-config.db');</span>
...
&nbsp;
                <span class="missing-if-branch" title="else path not taken" >E</span>if (!doc) {
                    doc = {
...
&nbsp;
<span class="fstat-no" title="function not covered" >    re</span>layUnsentTxs() {
<span class="cstat-no" title="statement not covered" >        return Promise.all([getGasPrice(this.config, true), getGasPrice(this.config, false)])</span>
            .then(
<span class="fstat-no" title="function not covered" >                ([</span>homeGP, foreignGP]) =&gt;
...
<span class="cstat-no" title="statement not covered" >                        this.db.txs.find({ status: 'to-send' }, <span class="fstat-no" title="function not covered" >(e</span>rr, docs) =&gt; {</span>
<span class="cstat-no" title="statement not covered" >                            if (err) {</span>
<span class="cstat-no" title="statement not covered" >                                logger.error('Error loading to-send txs');</span>
<span class="cstat-no" title="statement not covered" >                                resolve();</span>
                            }
&nbsp;
                            const promises = <span class="cstat-no" title="statement not covered" >docs.map(</span>
<span class="fstat-no" title="function not covered" >                                tx</span> =&gt;
<span class="cstat-no" title="statement not covered" >                                    tx.toHomeBridge</span>
                                        ? this.sendHomeTx(tx, homeGP)
                                        : this.sendForeignTx(tx, foreignGP),
                            );
<span class="cstat-no" title="statement not covered" >                            Promise.all([...promises]).then(<span class="fstat-no" title="function not covered" >()</span> =&gt; <span class="cstat-no" title="statement not covered" >resolve())</span>;</span>
                        });
                    }),
            )
            .catch(<span class="fstat-no" title="function not covered" >er</span>r =&gt; {
<span class="cstat-no" title="statement not covered" >                logger.error('Error sending unsent txs', err);</span>
<span class="cstat-no" title="statement not covered" >                sendEmail(</span>
                    this.config,
...
&nbsp;
                this.db.txs.insert(tx, (err, doc) =&gt; {
                    <span class="missing-if-branch" title="if path not taken" >I</span>if (err) {
<span class="cstat-no" title="statement not covered" >                        logger.error('Error inserting bridge-txs.db -&gt;', err, data);</span>
...
&nbsp;
    updateTxData(data) {
        const { _id } = data;
        <span class="missing-if-branch" title="if path not taken" >I</span>if (!_id) <span class="cstat-no" title="statement not covered" >throw new Error('Attempting to update txData without an _id');</span>
        this.db.txs.update({ _id }, data, {}, err =&gt; {
            <span class="missing-if-branch" title="if path not taken" >I</span>if (err) {
<span class="cstat-no" title="statement not covered" >                logger.error('Error updating bridge-txs.db -&gt;', err, data);</span>
            }
        });
    }
&nbsp;
    updateBridgeData(data) {
        this.db.bridge.update({ _id: data._id }, data, { upsert: true }, err =&gt; {
            <span class="missing-if-branch" title="if path not taken" >I</span>if (err) <span class="cstat-no" title="statement not covered" >logger.error('Error updating bridge-config.db -&gt;', err, data);</span>
        });
    }
}
&nbsp;</pre></td></tr>
...
<div class='footer quiet pad2 space-top1 center small'>
  Code coverage
  generated by <a href="https://istanbul.js.org/" target="_blank">istanbul</a> at Mon Jul 02 2018 16:51:41 GMT-0700 (PDT)
</div>
</div>
